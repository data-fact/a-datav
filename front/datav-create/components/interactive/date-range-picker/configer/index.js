...
import DatePickerConfiger from "../../date-picker/configer/DatePickerConfiger";
...
export default function Configer({component}) {
    ...
    useEffect(() => storePlaceholder && setPlaceholder(storePlaceholder), [storePlaceholder])
    useEffect(() => storeDefaultValue && setDefaultValue(storeDefaultValue), [storeDefaultValue])
    ...
    return (
        <>
            ...
            <Row style={{marginBottom: 8}}>
                ...
                <Col span={8}>
                    <Input
                        placeholder="开始提示语" size="small"
                        value={placeholder[0]}
                        onChange={e => setPlaceholder([e.target.value,placeholder[1]])}
                        onBlur={handlePlaceholderBlur}
                    />
                </Col>
                <Col span={8}>
                    <Input
                        placeholder="结束提示语" size="small"
                        value={placeholder[1]}
                        onChange={e => setPlaceholder([placeholder[0],e.target.value])}
                        onBlur={handlePlaceholderBlur}
                    />
                </Col>
            </Row>
            <Row style={{marginBottom: 8}}>
                ...
                <Col span={18}>
                    <Row>
                        <Col span={10}>
                            <Input
                                placeholder="开始默认值" size="small"
                                value={defaultValue[0]}
                                onChange={e => setDefaultValue([e.target.value,defaultValue[1]])}
                                onBlur={handleDefaultValueBlur}
                            />
                        </Col>
                        <Col span={10}>
                            <Input
                                placeholder="结束默认值" size="small"
                                value={defaultValue[1]}
                                onChange={e => setDefaultValue([defaultValue[0],e.target.value])}
                                onBlur={handleDefaultValueBlur}
                            />
                        </Col>
                    </Row>
